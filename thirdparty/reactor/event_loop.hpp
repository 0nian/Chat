...
public:
    ...
    rq_(rq),                     // 设置环形队列
    tm_(new TimerManager(600))   // IM 长连接：空闲超时放宽到 600 秒
    {}
                      ...
                      bool is_listensock = false)  // 是否监听socket
    {
        ...
        if(!is_listensock) tm_->Push(sock);
        // 添加到连接映射表
    ...
    void Recv(std::weak_ptr<Connection> connect)
    {
        ...
        while(true)
        {
            ...
            {
                lg(Info, "client [%s: %d] peer closed (recv=0)", connection->ip_.c_str(), connection->port_);
            ...
            else  // 接收出错
            {
                ...
                {
                    lg(Error, "client [%s: %d] recv error errno=%d", connection->ip_.c_str(), connection->port_, errno);
    ...
    void Send(std::weak_ptr<Connection> connect)
    {
        ...
        while(true)
        {
            ...
            else  // 发送出错
            {
                ...
                {
                    lg(Error, "client [%s: %d] send error errno=%d", connection->ip_.c_str(), connection->port_, errno);
                    connection->except_cb(connection);
                    return;
                }
            }
    ...
    void Expired_check()
    {
        ...
        {
            const int sockfd = tm_->GetTopFd();
            auto it = connections_.find(sockfd);
            if(it == connections_.end()) {
                tm_->LazyDelete(sockfd);
                continue;
            }
            auto c = it->second;
            lg(Warning, "client [%s: %d] idle timeout, force close", c->ip_.c_str(), c->port_);
            c->except_cb(c);
        }
    }
